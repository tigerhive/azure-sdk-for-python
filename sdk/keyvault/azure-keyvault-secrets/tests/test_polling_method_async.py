...
from azure.keyvault.secrets._shared._polling_async import AsyncKeyVaultPollingMethod
import pytest

from _shared.helpers import mock, mock_response
from _shared.helpers_async import get_completed_future

SLEEP = AsyncKeyVaultPollingMethod.__module__ + ".asyncio.sleep"
...
async def test_initialized_finished():
    ...
    polling_method = AsyncKeyVaultPollingMethod(command, final_resource=None, finished=True)
...
async def test_continues_polling_when_resource_not_found():
    ...
    polling_method = AsyncKeyVaultPollingMethod(command, final_resource=None, finished=False)
...
async def test_run_idempotence():
    ...
    resource = object()
    polling_method = AsyncKeyVaultPollingMethod(command, final_resource=resource, finished=False)
...
async def test_final_resource():
    ...
    assert AsyncKeyVaultPollingMethod(command=None, final_resource=resource, finished=True).resource() is resource
